...
if (__name__ == "__main__"):
    ...
    parser.add_argument("-pb", "--predict_batch", nargs=2, help="Loads existing model to predict on directory of images. Takes 2 args -p model_name img_dir")
    parser.add_argument("-cc", "--coco2dataset", nargs=1, help="Convert COCO JSON dataset to image and gt-dots. -cc json_path")
    parser.add_argument("-cd", "--gtmask2gtdot", nargs=1, help="Convert GT mask to GT dots. -cd gt-path")
    parser.add_argument("-sh", "--showhistory", nargs=1, help="Show model's training history. -sh history_json_path")
    parser.add_argument("-km", "--kmeansegment", nargs=2, help="")
    ...
    if (args.coco2dataset):
        from utilities import cocojson_to_dataset
        args = args.coco2dataset
        cocojson_to_dataset(args[0])
